...
class IQL:
    ...
    def train(self, **kwargs):
        ...
        for _ in range(n_episodes):
            ...
            while not done:
                observations = self.env.get_observations()  # (n_agents, observation_size)
                ...
                for agent in range(self.n_agents):
                    ...
                    def loss_fn(model, obs, actions, targets):
                        ...
                        actions = mini_batch["act"].astype(jnp.int32)  # (batch, 1)
                        q_selected = jnp.take_along_axis(q_values, actions, axis=1).squeeze()
                        loss = jnp.mean((q_selected - targets) ** 2)  # TODO: make generic
                        return loss
