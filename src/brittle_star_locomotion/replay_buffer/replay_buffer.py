...
class ReplayBuffer:
    def __init__(self, size_observation: int, size_action: int, max_size: int = 1000000, rng=jax.random.PRNGKey(0)) -> None:
    ...
    def add(self, observation: jnp.ndarray, action: jnp.ndarray, reward: float, next_observation: jnp.ndarray, done: bool) -> None:
        ...
        :type done: bool
        """
        new_entry = jnp.concatenate([observation, action, jnp.array([reward]), next_observation, jnp.array([done])])
    ...
    def sample(self, batch_size: int) -> jnp.ndarray:
        ...
        :rtype: jnp.ndarray
        """
        random_indices = jax.random.choice(self.rng, self.size, shape=(batch_size,), replace=False)
