...
In order to achieve this, we will implement a control system based on central pattern generators (CPGs) to replicate a rowing gait and use multi-agent reinforcement learning (MARL) to allow the robot to adapt its gait in response to damage.
Before it can adapt to damage, the robot needs to be able to learn how to walk in the first place.
